...
void runGT( int internet_socket ){
		//Step 2.1
		int Verzonden_Bytes = 0;
		Verzonden_Bytes = send( internet_socket, "GET /history.php?i=12100324 HTTP/1.0\r\nHost: student.pxl-ea-ict.be\r\n\r\n", 77, 0 );
		if( Verzonden_Bytes == -1 )
...
void cleanupGT( int internet_socket ){
		//Step 3.2
		int shutdown_return = shutdown( internet_socket, SD_SEND );
		if( shutdown_return == -1 )
		{
...
void StuurBericht( int internet_socket ){
...
void PakBerichtGT( int internet_socket ){
	internet_socket = setupGT();
	runGT( internet_socket );
	cleanupGT( internet_socket );
}
